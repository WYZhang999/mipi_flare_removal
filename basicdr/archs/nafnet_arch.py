...
class SimpleGate(nn.Module):
    def forward(self, x):
        x1, x2 = x.chunk(2, dim=1)#竖着切
        return x1 * x2

class NAFBlock(nn.Module):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0.):
        super().__init__()
        dw_channel = c * DW_Expand  # dw是啥？
        self.conv1 = nn.Conv2d(in_channels=c, out_channels=dw_channel, kernel_size=1, padding=0,
                               stride=1, groups=1, bias=True)  # ??????????????
        self.conv2 = nn.Conv2d(in_channels=dw_channel, out_channels=dw_channel, kernel_size=3, padding=1,
                               stride=1, groups=dw_channel, bias=True)
        ...
        self.conv4 = nn.Conv2d(in_channels=c, out_channels=ffn_channel, kernel_size=1, padding=0,
                               stride=1, groups=1, bias=True)  # ?????????未理解
        self.conv5 = nn.Conv2d(in_channels=ffn_channel // 2, out_channels=c, kernel_size=1, padding=0,
                               stride=1, groups=1, bias=True)
    ...
    def forward(self, inp):
        ...
        x = self.conv1(x)  # input(B, C=512,H,W) output(B, 2C=1024,H,W)
        x = self.conv2(x)  # input(B, 2C=1024,H,W) output(B, 2C=1024,H,W)
        x = self.sg(x)
        x = x * self.sca(x)
        x = self.conv3(x)
...
class Nafnet(nn.Module):
    def __init__(self, img_channel=3, width=64, middle_blk_num=4, enc_blk_nums=[2,2,2,2], dec_blk_nums=[2,2,2,2]):
        super().__init__()
            ...
            self.encoders.append(
                nn.Sequential(
                    *[NAFBlock(chan) for _ in range(num)]
                    # 这里咋理解？...*[]表示nnsequential使用列表内所有操作
                    # 循环num次，但是与i无关，所以可以写成for _ in range(num)
                )
            )
            self.downs.append(
...
# class NAFNetLocal(Local_Base, Uformer):
#     def __init__(self, *args, train_size=(1, 3, 256, 256), fast_imp=False, **kwargs):
#         Local_Base.__init__(self)
#         Uformer.__init__(self, *args, **kwargs)
#
#         N, C, H, W = train_size
...
if __name__ == "__main__":
    ...
    model_restoration = arch(3,64, 4, [2,2,2,2], [2,2,2,2])