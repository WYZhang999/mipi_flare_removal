...
def predict_flare_from_3_channel(input_tensor,flare_mask,base_img):
    #,merge_img,gamma):#flare_img,
...
def remove_flare(combined, flare, gamma = 2.2):
    ...
    # gamma < 1.
    combined = torch.clip(combined, _EPS, 1.0)
    flare = torch.clip(flare, _EPS, 1.0)
    ...
    # Avoid zero. Otherwise, the gradient of pow() below will be undefined when
    # gamma > 1.
    scene_linear = torch.clip(scene_linear, _EPS, 1.0)
    scene = torch.clip(scene_linear, 1.0 / gamma)
    return scene
