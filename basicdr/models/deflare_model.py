...
class DeflareModel(SRModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        # print(self.output.shape, self.output.mean())
        # save_image = torch.cat((self.lq, self.flare, self.output, self.gt), dim=-1)
        # for idx in range(self.output.shape[0]):
        #     img = save_image[idx]
        #     img = tensor2img(img).astype(np.uint8)
        #     imageio.imwrite(f'./{current_iter}_{idx}.jpg', img)
        ...
        if self.output_ch==6:
            ...
            # self.deflare1, self.flare_hat1 = predict_flare_from_3_channel(self.output_sta1, self.mask,
            #                                                               self.lq, self.flare,self.lq,self.gamma)
        elif self.output_ch==3:
            # self.pred_flare = remove_flare(self.lq, self.output, 2.2)
            # self.mask = get_highlight_mask(self.flare).cuda()
            ...
            #                                                          self.lq,self.gamma)
            # self.deflare = self.output * (1 - self.mask) + self.gt * self.mask
            # self.flare_hat = self.pred_flare * (1 - self.mask) + self.flare * self.mask
            self.mask = torch.zeros_like(self.lq).cuda()  # Comment this line if you want to use the mask
            self.deflare = predict_flare_from_3_channel(self.output, self.mask, self.lq)
        else:
        ...
        # l1 loss
        # save_image = torch.cat((self.flare_hat, self.flare, self.deflare, self.gt), dim=-1)
        # for idx in range(self.output.shape[0]):
        #     img = save_image[idx]
        #     img = tensor2img(img).astype(np.uint8)
        #     imageio.imwrite(f'./{current_iter}_{idx}.jpg', img)

        # l1_flare = 0#self.l1_pix(self.flare_hat, self.flare)
        ...
        # perceptual loss
        # l_vgg_flare = 0#self.l_perceptual(self.flare_hat, self.flare)
    ...
    def test(self):
        ...
        if self.output_ch==6:
            ...
            # self.mask = torch.zeros_like(self.lq).cuda()  # Comment this line if you want to use the mask
            # self.deflare1, self.flare_hat1 = predict_flare_from_3_channel(self.output_sta1, self.mask,
            #                                                             self.gt,self.flare,self.lq,self.gamma)
        elif self.output_ch==3:
            self.mask=torch.zeros_like(self.lq).cuda() # Comment this line if you want to use the mask
            self.deflare=predict_flare_from_3_channel(self.output,self.mask,self.lq)#self.flare,
                                                                     #self.lq)#,self.gamma)
        else:
            assert False, "Error! Output channel should be defined as 3 or 6."
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        if with_metrics:
            for metric in self.metric_results.keys():
                self.metric_results[metric] /= 2000#(idx + 1)
